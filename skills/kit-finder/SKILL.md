---
name: kit-finder
description: "Find specific reagent kit part numbers and catalog numbers for laboratory protocol steps. Asks the user first for their preferred kits; if not provided, searches vendor websites for options with catalog numbers and direct product links, then asks for user confirmation before finalizing."
version: 1.0.0
user-invocable: true
argument-hint: "<protocol-steps or reagent list> [--vendor 'preferred vendor']"
allowed-tools:
  - WebSearch
  - WebFetch
  - Read
...
3. **Verify product links** — Use WebFetch on product URLs when possible to confirm:
...
2. **Download each PDF** using WebFetch to the project's `references/kit-docs/` directory:
